...
library(ggplot2)
PRC <- read.table("/home/xielucheng/Documents/DELL-G5_syncthing/Work_progress/2020/2020_11_13_CCKChR2_LL-06/CCKAi32_LL_PRC.txt",header = T)

prc <- as.data.frame(PRC)
ggplot(prc,aes(x=stimTime,y=phaseChange)) +
  geom_point() +
  geom_smooth() +
...
#####################  wavelet analysis
library(WaveletComp)
ll_Tb <- read.delim("/home/xielucheng/Documents/DELL-G5_syncthing/Work_progress/2020/2020_11_13_CCKChR2_LL-06/CCKxAi32_LL_Tb.txt",header = T)
Tb <- data.frame(x=ll_Tb[,3:10])
##plot(Tb[,1])
a <- gl(8064,5)
A <- Tb[1:40320,2]
TT <- tapply(A,a,mean)
###############  downsample  1min/bin----->5min/bin
a1 <- data.frame(x=TT)
Tb1 <- analyze.wavelet(a1,"x",loess.span = 0,dt = 1/288,lowerPeriod = 0.75,upperPeriod = 100)
